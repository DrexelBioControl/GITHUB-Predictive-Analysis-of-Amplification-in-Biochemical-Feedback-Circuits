...
def build_control_dataset(ws):
    """
    Build 0 input / 0 fuel dataset.

    Excel layout:
    column 0 = time
    column 1 = 0 input / 0 fuel normalized reporter signal
    """
    ...
    y_norm = np.array(
        [ws.cell_value(r, 1) for r in range(START_ROW, ws.nrows)],
        dtype=float
    )
...
def predict(model, dataset, params):
    """Simulate one model for the 0 input / 0 fuel condition."""
...
plt.title("0 input / 0 fuel control condition")
plt.grid(True)
